...
const TodoList = (props) => {
  ...
  return (
    <div className='bg-zinc-700 text-white flex flex-col items-center'>
        <h2>Here's what you need to do</h2>
        {
          props.items.length === 0? 
            ...
            <div className='flex flex-col items-center'>
              <img src="/robot-with-404-error-free-vector-removebg-preview.png" alt="" className='w-3xs h-3xs'/>
              <br/>
              <p className='text-center'>Sorry Nothing todo's for {props.selectedDate.toDateString()}</p>
            </div>
          ):

        (<ul className=''>
            {props.items.filter((item) => item.date === props.selectedDate.toDateString()).map((item, index)=>
            <li className='flex flex-row' key={index}>{index + 1}.  {item.name} - {item.description}
        ...
        <button className='bg-blue-500 rounded-lg p-2 hover:bg-blue-700 mt-7 mb-2' onClick={handleAddTodo}>Add Todo</button>
    </div>
  )
}
